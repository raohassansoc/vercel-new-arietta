...
import UserListedNFTCard from './UserListedNFTCard'
...
const Collectible = () => {
  ...
  const [getAllNFTsNonListed, setGetAllNFTsNonListed] = useState([]);
  const [getAllNFTsListed, setGetAllNFTsListed] = useState([]);


  useEffect(() => {
    console.log("my wwwwwwwwwwwwwwwwwwwwwwwwwww ===> ", getAllNFTsListed)
  }, [getAllNFTsListed])

  const NFTResult = useContractRead({
    address: '0x1b61EEb2529F89F764959F329102Bd3604B9a8Bf',
    abi: ABI,
    functionName: 'getAllNFTsByUser',
    args: [address]
  } as any)
  ...
  useEffect(() => {
    try {
      ...
      setGetAllNFTsNonListed(allNFTs[1] as any)
      setGetAllNFTsListed(allNFTs[0] as any)
    } catch (error) {
      console.log(error)
    }
            ...
            {viewMode === ViewMode.TABLE ? (
              ...
              <>
                ...
                <Grid
                  ...
                  data-test="nft-collection-row"
                > {
                    getAllNFTsNonListed.map((nft, ind) => {
                ...
                <h1 style={{ paddingBottom: "20px", marginTop: "20px", color: "#fcb016", fontSize: "22px" }}>My listed NFTs</h1>
                <Grid
                  gridGap="16px"
                  gridTemplateColumns={['1fr', '1fr', 'repeat(2, 1fr)', 'repeat(3, 1fr)']}
                  mb="32px"
                ...
                >
                  {
                    getAllNFTsListed.map((nft, ind) => {

                      return (

                        <div key={ind}>
                          <UserListedNFTCard nft={nft} />
                        </div>
