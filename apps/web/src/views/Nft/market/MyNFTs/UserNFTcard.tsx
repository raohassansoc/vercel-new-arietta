...
import NFTDetailModal from '../Home/NFTDetailModal';
...
const NFTCard = ({ nft }) => {
    ...
    const { write, isLoading } = useContractWrite({
        ...
        args: [
            ...
            BigInt(tokenPrice) * BigInt(10 ** 18)
            // BigInt(tokenPrice as any * 10000000000000000000)
        ],
    ...
    useEffect(() => {
        const num = BigInt(tokenPrice) * BigInt(10 ** 18)
        console.log("my logggg ", BigInt(tokenPrice) * BigInt(10 ** 18))
        // setTokenPriceWei(weiValue as any)
    }, [tokenPrice])
        ...
        <div style={styles.cardStyle} >
            ...
            {/* <BuyNFTModal isOpen={ISopenBuy} onClose={closeModalBuy} /> */}
            {
                ISopenBuy == true ? <>
                    <div style={styles.overlayStyle}
                        onClick={() => { setISopenBuy(false); setTokenID(""); setTokenPrice(0) }}
                    />
                    <div style={styles.modalStyle}>
                        <h1 style={{ color: "black", marginBottom: "10px" }}>List NFT</h1>
                        <input style={{ marginBottom: "10px" }} type="number" placeholder='Price' onChange={(e) => setTokenPrice(e.target.value as any)} /> <br />
                        <button onClick={() => write({
                        } as any)}>List NFT</button>
                        {
            ...
            {/* {
                nft?.isListed ? <div style={styles.priceStyle}>
                    <p>price:
                        {nft?.price?.toNumber()}
                        ASC</p> <button onClick={() => write({
                        } as any)} style={styles.btnWrapper}>Buy</button>
                </div> : ""
            } */}
