...
import { ARIETTA_BLOCK_TIME } from 'config'
  ...
  const fetchIfoData = useCallback(
    async (currentBlock: number) => {
      ...
      setState((prev) => ({
        ...prev,
        isInitialized: true,
        secondsUntilEnd: blocksRemaining * ARIETTA_BLOCK_TIME,
        secondsUntilStart: (startBlockNum - currentBlock) * ARIETTA_BLOCK_TIME,
        poolBasic: {
          ...poolBasicFormatted,
          taxRate: 0,
