...
export function useAllTokens(): { [address: string]: ERC20Token } {
  ...
  console.log("my Active Chain ID ===> ", chainId)
...
export function useToken(tokenAddress?: string): ERC20Token | undefined | null {
  const { chainId } = useActiveChainId()
  console.log("BBBBBBBBBBBBBBBBIIIIGGGGGGGGGG FFFFFNNNNNNNNN RUnnning ====> ", chainId)
  console.log("TOKEN ADDRESS FOR  USE TOKEN ====> ", tokenAddress)
...
export function useCurrency(currencyId: string | undefined): Currency | ERC20Token | null | undefined {
  // currencyId = '92522';
  console.log("My Native Currency is coming from here!", currencyId)
  const native = useNativeCurrency()
  //  alert(currencyId?.toUpperCase());
  const isNative =
    currencyId?.toUpperCase() === native.symbol?.toUpperCase() || currencyId?.toLowerCase() === GELATO_NATIVE
  const token = useToken(isNative ? undefined : currencyId)
  //  const token = useToken('92522')

  console.log("IIIIIXXXXXXXXXXXXXXXXX ====> ", currencyId)
  console.log("IIIIISSSSSS Native =====> ", isNative)
